...
parser.add_argument('--recording_file', type = str,default='MLP_Mixer_5_17-22_45_FL')
...
parser.add_argument('--milestones', type = list, default=[500,700])
parser.add_argument('--LR', type = float, default=0.15)
parser.add_argument('--MODEL', default='MLPMixer') # 现在只提供了MLPMixer和resnet34两个选项
...
parser.add_argument('--loss_func', default=FocalLoss(num_classes=8, alpha=None, gamma=2))
...
def main():
    ...
    for epoch in range(0, args.MAX_EPOCH):
        ...
        loss_valid, acc_valid, mat_valid, y_true_valid, y_outputs_valid =\
            ModelTrainer.valid(valid_loader, MODEL,criterion, device)
        # staticmethod不需要实例化 每训练一个epoch就在validation上面测试一次结果 相当于是用这个validation set来监视防止过拟合的
